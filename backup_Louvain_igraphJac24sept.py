...
def make_csrmatrix_noselfloop(neighbor_array, distance_array, dist_std=0, keep_all=False):
    ...
    for row in neighbor_array:
        ...
        if keep_all_dist== False:
            ...
            if discard_count!=0: print(discard_count)
...
def run_toobig_sublouvain(X_data, knn_struct, k_nn, self_loop=False, keep_all=False,
                          Jac_std=0.3):  # 0.3 was default dist_std
    n_elements = X_data.shape[0]
    time_start_knn = time.time()
...
def run_sublouvain(X_data, knn_struct, k_nn, self_loop=False, too_big_factor=0.15, small_pop=5, dist_std=0,
                   keep_all=False, Jac_std=0.3):  # 50
    ...
    print(len(sim_list), 'is length of simlist')
    # print('pre-lognormed mean and std', np.mean(sim_list), np.std(sim_list))
    # plt.hist(sim_list)
    ...
    time_end_prune = time.time() - time_start_sub
    time_start_louvain = time.time()
    print('starting Louvain clustering at', time.ctime())
    partition = louvain.find_partition(G_sim, louvain.ModularityVertexPartition)  # , weights='weight')
    louvain_labels = np.empty((n_elements, 1))
    ...
    return louvain_labels, undirected_graph_array_forLV, time_end_louvain, time_end_prune


def run_mainlouvain(X_data, true_label, self_loop=False, LV_graphinput_file_name=None, LV_plot_file_name=None,
                    too_big_factor=0.4, knn_in=30, small_pop=50, dist_std=0, keep_all=True, Jac_std=0.3):
    list_roc = []
    ...
    for k_nn in k_nn_range:
        ...
        louvain_labels, undirected_graph_array_forLV, time_end_louvain, time_end_prune = run_sublouvain(X_data,
                                                                                                        knn_struct,
                                                                                                        k_nn,
                                                                                                        self_loop=self_loop,
        ...
        for onevsall_val in targets:
            ...
            vals_roc, predict_class_array, majority_truth_labels = accuracy_mst(list(louvain_labels.flatten()),
                                                                                true_label,
    ...
    return predict_class_aggregate, df_accuracy, temp_best_labels, knn_opt, onevsall_opt, majority_truth_labels, time_end_knn, time_end_prune, time_end_louvain, time_end_total, f1_accumulated, f1_mean
...
def run_phenograph(X_data, true_label):
    ...
    f1_acc_noweighting = 0
    for onevsall_val in targets:
        vals_roc, predict_class_array, majority_truth_labels = accuracy_mst(list(communities), pd.Series(true_label),
                                                                            None, 'phenograph',
                                                                            onevsall=onevsall_val)
...
def accuracy_mst(model, true_labels, embedding_filename, clustering_algo, phenograph_time=None, onevsall=1,
    ...
    for kk in sorted_keys:
        ...
        # Vif majority_val == onevsall: print('cluster',kk, ' has majority', onevsall, 'with population', len(vals))
    ...
    # Vprint('num groups', 'error rate', 'onevsall', 'population', 'f1_score', 'fnr ', 'sigma', ' min cluster size', 'mergetooclose factor',  len(sorted_keys), error_rate, onevsall, n_cancer, f1_score, fnr)
    ...
    return accuracy_val, predict_class_array, majority_truth_labels
