...
def make_csrmatrix_noselfloop(neighbor_array, distance_array,dist_std =0, keep_all_dist= False):
    ...
    for row in neighbor_array:
        ...
        #print(distlist)
        if keep_all_dist== False: to_keep = np.where(distlist < np.mean(distlist)+dist_std*np.std(distlist))[0] #0*std
        if keep_all_dist== True: to_keep = np.where(distlist < np.max(distlist))[0]
...
def run_toobig_sublouvain(X_data, knn_struct,k_nn,self_loop = False, keep_all=False):
    ...
    if self_loop == False:
        csr_array, undirected_graph_array_forLV = make_csrmatrix_noselfloop(neighbor_array, distance_array, keep_all=keep_all)
...
def run_sublouvain(X_data, knn_struct,k_nn,self_loop = False, too_big_factor = 0.4, small_pop = 50, dist_std = 0):
    n_elements = X_data.shape[0]
    time_start_knn = time.time()
    X_data_copy = copy.deepcopy(X_data)
    ...
    print('simlist jaccard weights mean and std:',np.mean(sim_list),np.std(sim_list), sim_list)
    G_sim = ig.Graph(list(edgelist), edge_attrs={'weight': sim_list})
    ...
    while too_big == True:
        knn_big = 50 #100 for 8th sep
        #ef_big = knn_big + 10
        X_data_big = X_data[cluster_big_loc,:]
        knn_struct_big = make_knn_struct(X_data_big)
        print(X_data_big.shape)
        louvain_labels_big = run_toobig_sublouvain(X_data_big, knn_struct_big, k_nn=knn_big, self_loop=False) #knn=200 for 10x
...
def run_mainlouvain(X_data,true_label, self_loop = False,LV_graphinput_file_name =None, LV_plot_file_name =None, too_big_factor = 0.4, knn_in = 30,small_pop = 50,dist_std = 0):
    list_roc = []
    ...
    for k_nn in k_nn_range:
        ...
        louvain_labels, undirected_graph_array_forLV = run_sublouvain(X_data, knn_struct, k_nn, self_loop=self_loop, too_big_factor=too_big_factor, small_pop = small_pop, dist_std = dist_std)
    ...
    return predict_class_aggregate, df_accuracy, temp_best_labels,knn_opt, onevsall_opt, majority_truth_labels
