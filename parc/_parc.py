...
class PARC:
    def __init__(self, data, true_label='NA', too_big_factor = 0.4, small_pop = 10,dist_std_local = 2,dist_std_global = 3,jac_std_global='median', keep_all_local_dist = False,jac_weighted_edges = True,knn = 30, n_iter_leiden=5):
    ...
    def run_PARC(self):
        ...
        if self.true_label == 'NA': self.true_label = [1]*self.data.shape[0]
        list_roc = []
        ...
        if len(targets)>1:
            ...
            for onevsall_val in targets:
                print('one vs all in targets', onevsall_val)
