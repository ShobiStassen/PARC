...
def get_data(cancer_type, benign_type, n_cancer, ratio, fluor, dataset_number, new_folder_name, method,
    ...
    new_file_name = new_file_name_title = 'N' + str(n_total) + '_r{:.2f}'.format(
        ...
        dataset_number)
    if method == 'bh':
        label_file_name = '/home/shobi/Thesis/BarnesHutMC_data_MinCluster/' + new_folder_name + '/BH_label_' + new_file_name + '.txt'
        tag_file_name = '/home/shobi/Thesis/BarnesHutMC_data_MinCluster/' + new_folder_name + '/BH_tag_' + new_file_name + '.txt'
        data_file_name = '/home/shobi/Thesis/BarnesHutMC_data_MinCluster/' + new_folder_name + '/BH_data_' + new_file_name + '.txt'
    if method == 'lv':
        label_file_name = '/home/shobi/Thesis/LV_data_MinCluster/' + new_folder_name + '/LV_label_' + new_file_name + '.txt'
        tag_file_name = '/home/shobi/Thesis/LV_data_MinCluster/' + new_folder_name + '/LV_tag_' + new_file_name + '.txt'
        data_file_name = '/home/shobi/Thesis/LV_data_MinCluster/' + new_folder_name + '/LV_data_' + new_file_name + '.txt'
    ...
    if benign_type == 'pbmc':
        ...
        pbmc_fileidx.columns = ['filename',
                                'matlabindex']  # the column number of the cell in the entire cellparameters matrix. these column numbers pick out the gated cells
...
def simple_accuracy(predicted_labels, true_labels, data_version, index_list, df_all, lr, tsne_version):
    ...
    column_names_tags = ['celltype_' + unique_version, 'filename_' + unique_version, 'idx_inmatfile_' + unique_version,
                         'File ID_' + unique_version, 'Cell ID_' + unique_version, 'df_all idx_' + unique_version]
    ...
    for k in range(n_tot):
        ...
        if (predicted_labels[k] == 1) and (true_labels[k] == 1):
            tp = tp + 1
    if len(fp_accum_list) > 0: df_fptemp = pd.concat(
        [pd.DataFrame([i], columns=column_names_tags) for i in fp_accum_list], ignore_index=True)
    if len(fn_accum_list) > 0: df_fntemp = pd.concat(
        [pd.DataFrame([i], columns=column_names_tags) for i in fn_accum_list], ignore_index=True)
    ...
    summary_simple_acc = [data_version, n_cancer, n_pbmc, f1_score, tnr, fnr, tpr, fpr, precision, recall, error_rate,
                          computed_ratio]
    print('summary simple_acc', summary_simple_acc)
    print(tn, 'tn')
    print(tn, fn, tp, fp)
    print('completed accuracy of mode calculation, the time is now', time.ctime(),
          'time elapsed is: {} seconds'.format(time.time() - time_start))
...
def run_lv(version, input_data, perplexity, lr, new_file_name, new_folder_name):
    ...
    if new_folder_name != None:
        embedding_filename = '/home/shobi/Thesis/LV_data_MinCluster/' + new_folder_name + '/LV_embedding' + new_file_name + '_lr' + str(
...
def run_mctsne(version, input_data, perplexity, lr, new_file_name, new_folder_name):
    ...
    embedding_filename = '/home/shobi/Thesis/BarnesHutMC_data_MinCluster/' + new_folder_name + '/BH_embedding' + new_file_name + '_lr' + str(
        lr) + '_perplexity' + str(perplexity) + '_bh' + str(version) + '.txt'
    tsne = multicore_tsne(n_jobs=8, perplexity=perplexity, verbose=1, n_iter=1000, learning_rate=lr, angle=0.2)
...
def run_kmeans(X_embedded, n_cancer, true_label, init_peaks, data_version, tsne_version, embedding_filename,
               tsne_runtime):
...
def run_dbscan(X_embedded, n_cancer, true_label, data_version, tsne_version, embedding_filename, tsne_runtime):
    ...
    list_roc = []
    sigma_list = [
        0.5]  # eps=0.5 is the default, we are setting eps as mu+2.5*std of edge distance in mst_graph of embedding
    tooclosefactor_list = [0]  # ,5,0]
    if n_cancer > 1000:
...
def run_mstclustering(X_embedded, n_cancer, true_label, data_version, tsne_version, embedding_filename, tsne_runtime):
    ...
    # sigma_list = [3,2.5,2]
    # tooclosefactor_list =[60, 50,40,30,25,20,15,10,0]
    if n_cancer > 1000:
        cluster_size_list = [20]
        min_cluster_size = 20
    else:
    ...
    print('Clustering Done! Time elapsed: {:.2f} seconds'.format(mst_runtime),
          'tsne version {}'.format(tsne_version), 'data version {}'.format(data_version),
          'sigma {}'.format(model.sigma_factor), 'tooclosefactor {}'.format(model.tooclosefactor),
          'and min cluster {}'.format(min_cluster_size))
    ...
    df_accuracy = pd.DataFrame(list_roc,
                               columns=['embedding filename', 'sigma', 'min cluster size', 'merge-too-close factor',
                                        'f1-score', 'tnr', 'fnr',
                                        'tpr', 'fpr', 'precision', 'recall', 'num_groups', 'clustering runtime',
                                        'tsne runtime'])
...
def accuracy_mst(model, true_labels, embedding_filename, clustering_algo, phenograph_time=None):
    ...
    print('f1_score', 'fnr ', 'sigma', ' min cluster size', 'mergetooclose factor', f1_score, fnr, sigma,
          min_cluster_size, mergetooclosefactor)
    if clustering_algo == 'phenograph':
    ...
    if clustering_algo == 'louvain':
        accuracy_val = [f1_score, tnr, fnr, tpr, fpr, precision,
                        recall, num_groups]
    else:
        accuracy_val = [embedding_filename, sigma, min_cluster_size, mergetooclosefactor, f1_score, tnr, fnr, tpr, fpr,
                        precision,
...
def plot_mst_simple(ax0, ax1, model, true_labels, embedding_filename, embedding_plot_title, sigma, min_cluster,
    ...
    ax0.scatter(X_plot[idx_cancer, 0], X_plot[idx_cancer, 1], color='red', zorder=2, alpha=0.5, s=6)
    ax0.scatter(X_plot[idx_benign, 0], X_plot[idx_benign, 1], color='blue', zorder=2, alpha=0.5, s=6)
    ...
    for color_p, ll_p in zip(colors_pbmc, pbmc_labels):
        ...
        ax1.scatter(x, y, color=color_p, s=6, alpha=0.5, label='pbmc ' + str(ll_p) + ' Cellcount = ' + str(len(x)))
    ...
    for color_t, ll_t in zip(colors_thp1, thp1_labels):
        ...
        ax1.scatter(x, y, color=color_t, s=6, alpha=0.5,
    ...
    if clustering_method == 'mst': title_str1 = 'MST: mean + ' + str(
        sigma) + '-sigma cutoff and min cluster size of: ' + str(
    ...
    if clustering_method == 'dbscan': title_str1 = 'DBSCAN eps = {:.2f}'.format(
        sigma) + ' and min cluster size of: ' + str(
        min_cluster) + '\n' + "error: " + " {:.2f}".format(error_rate * 100) + '%' + " FP: " + " {:.1f}".format(
        fp * 100 / n_pbmc) + "%. FN of " + "{:.1f}".format(
        fn * 100 / (n_cancer)) + '%' + '\n' + 'computed ratio:' + "{:.1f}".format(
        computed_ratio) + ' f1-score:' + "{:.2f}".format(f1_score * 100)
    if clustering_method == 'kmeans': title_str1 = 'KMEANS num centroids' + str(
        num_groups) + '\n' + "error: " + " {:.2f}".format(error_rate * 100) + '%' + " FP: " + " {:.1f}".format(
        fp * 100 / n_pbmc) + "%. FN of " + "{:.1f}".format(
        fn * 100 / (n_cancer)) + '%' + '\n' + 'computed ratio:' + "{:.1f}".format(
        computed_ratio) + ' f1-score:' + "{:.2f}".format(f1_score * 100)
    ...
    if clustering_method == 'phenograph' or clustering_method == 'louvain':
        ...
        mst_runtime = model.clustering_runtime_
    roc_val = [embedding_filename, sigma, min_cluster, tooclose_factor, f1_score, tnr, fnr, tpr, fpr, precision, recall,
               num_groups,
               mst_runtime]
...
def tsne_densityplot(X_embedded, ax1, ax2, df_all, mode='plot'):
    ...
    positions = np.vstack(
        [X.ravel(), Y.ravel()]).T  # nx2 matrix whre first column is x-coordin and 2nd is y-coordinates
    Z_tsne = np.reshape(np.exp(kde.score_samples(positions)), X.shape)
    # plot contours of the density
    peak_coord = plot_peaks(X, Y, positions, Z_tsne, ax1, ax2, density_type='tsne',
                            mode=mode)  # returns n_peak x2 array
...
def detect_peaks(image, density_type='features'):
    ...
    if density_type == 'features':
        ...
        threshold = average_intensity + 2 * std_intensity
        print('average intensity', average_intensity, 'threshold: ', threshold)
...
def plot_peaks(X, Y, positions, image, ax1, ax2, density_type, mode='plot'):
...
def run_main(cancer_type, n_cancer, n_benign, tsne_algo, randomseed=None):
    randomseed_val = None
    method = tsne_algo
    perplexity = 30
    n_cancer = n_cancer
    ...
    if method == 'lv':
        ...
        perp_range = [10, 30, 50, 70, 90]
    if method == 'bh':
        lr = 1000
        perplexity = 30
        lr_range = [lr]  # [100,1000,1500,2000,2500]
        perp_range =[30]# [10, 30, 50, 70, 90]
    new_folder_name = cancer_type + '_r{:.2f}'.format(ratio) + '_n' + str(n_cancer)
    if method == 'bh': path_tocreate = '/home/shobi/Thesis/BarnesHutMC_data_MinCluster/' + new_folder_name
    if method == 'lv': path_tocreate = '/home/shobi/Thesis/LV_data_MinCluster/' + new_folder_name
    os.mkdir(path_tocreate)

    lr_file = lr_range[0]
    for dataset_version in dataset_version_range:
        if method == 'lv':
            excel_file_name = '/home/shobi/Thesis/LV_data_MinCluster/' + new_folder_name + '/LV_excel_' + cancer_type + '_data' + str(
                dataset_version) + '_r{:.2f}'.format(ratio) + '_ncancer' + str(n_cancer) + '.xlsx'
        else:
            excel_file_name = '/home/shobi/Thesis/BarnesHutMC_data_MinCluster/' + new_folder_name + '/BH_excel_' + cancer_type + '_data' + str(
                dataset_version) + '_r{:.2f}'.format(ratio) + '_ncancer' + str(n_cancer) + '.xlsx'
        ...
        true_label, tag, X_data, new_file_name, df_all, index_list, flist = get_data(cancer_type, benign_type, n_cancer,
                                                                                     ...
                                                                                     new_folder_name, method=method,
                                                                                     randomseedval=randomseed_val)
        predict_class_aggregate_louvain, df_accuracy_louvain, best_louvain_labels, knn_opt = run_mainlouvain(X_data,
                                                                                                             true_label)
        ...
        for perplexity in perp_range:
            ...
            for tsne_version in tsne_version_range:
                ...
                # predict_class_aggregate_louvain, df_accuracy_louvain, best_louvain_labels, knn_opt = run_mainlouvain(
                #   X_embedded, true_label)
                ...
                peak_vals = tsne_densityplot(X_embedded, None, None, df_all, mode='only_vals')
                '''
                print('start KMEANS')
                ...
                roc_louvain, dummy_ax_all, ax_all[2][0] = plot_mst_simple(ax_all[0][0], ax_all[2][0],
                ...
                '''
                roc_mst, ax_all[0][0], ax_all[0][1] = plot_mst_simple(ax_all[0][0], ax_all[0][1], model_mst, true_label,
                                                                      embedding_filename, embedding_plot_title,
                                                                      sigma_opt,
                                                                      min_cluster_size_opt, cancer_type,
                ...
                if n_total < 500000:
                    model_dbscan = DBSCAN(eps_opt, min_cluster_size_opt_dbscan, tooclose_factor=tooclosefactor_opt).fit(
                        X_embedded)
                    # model_dbscan_3sigma = DBSCAN(eps_opt, min_samples = 20, tooclose_factor=20).fit(X_embedded)
                    roc_dbscan, dummy_ax, ax_all[1][0] = plot_mst_simple(ax_all[0][0], ax_all[1][0], model_dbscan,
                                                                         true_label, embedding_filename,
                                                                         embedding_plot_title,
                                                                         eps_opt, min_cluster_size_opt_dbscan,
            ...
            predict_class_final = predict_class_aggregate_all
        ...
        if n_total < 500000:
            ...
            # df_mode_dbscan.to_excel(writer,   'DBSCAN Mode')  # one mode per set of equal param tsne. so if there are 4 tsnes per Learning rate, then these 4 tsnes yield one mode
...
def main():
    ...
    # run_main('k562', 46, 46000, 'lv', randomseed=[21, 22,23])
    pb = 50 * 10
    # run_main('k562', 46, pb, 'bh', randomseed=[23, 21, 22])
    run_main('k562', 50, pb, 'bh', randomseed=[7, 8, 9])
    # run_main('k562', 100, 100000, 'lv', randomseed=[16, 17, 18])
