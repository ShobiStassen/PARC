...
def run_mctsne(version, input_data,perplexity, lr, new_file_name, new_folder_name):
    ...
    embedding_filename = '/home/shobi/Thesis/BarnesHutMC_data/'+new_folder_name+'/BH_embedding' +  new_file_name +'_lr'+str(lr)+'_bh'+str(version)+ '.txt'
...
def plot_mst_simple(model, true_labels, embedding_filename, embedding_plot_title, sigma, min_cluster,
    ...
    if merge == 1:
        ...
        if clustering_method == 'mst': ax[2].plot(segments[0], segments[1], '-k', zorder=1, linewidth=0.5)
...
def main():
    ...
    for dataset_version in dataset_version_range:
        ...
        for lr in lr_range:
            ...
            if n_total < 500000: pred_list_dbscan.append([lr] + summary_simple_acc_dbscan)
            df_fn_temp.to_excel(writer, 'fn tags'+'lr'+str(lr)+'d'+str(dataset_version), index=False)  # per lr (i.e. same frequency as mode)
            df_fp_temp.to_excel(writer, 'fp tags'+'lr'+str(lr)+'d'+str(dataset_version), index=False)
        ...
        df_mode = pd.DataFrame(pred_list,
                               columns=['learning rate', 'data version', 'n_cancer', 'n_pbmc', 'f1-score', 'tnr', 'fnr', 'tpr', 'fpr',
                                        'precision', 'recall', 'error_rate', 'computed_ratio'])
        if n_total < 500000: df_mode_dbscan = pd.DataFrame(pred_list_dbscan,columns=['learning rate', 'data version','n_cancer', 'n_pbmc', 'f1-score', 'tnr', 'fnr', 'tpr', 'fpr',
                                        'precision', 'recall', 'error_rate', 'computed_ratio'])
