...
new_folder_name =  'MultiClass'
#label_file_name = '/home/shobi/Thesis/'+ new_folder_name + '/BH_label_' + new_file_name + '.txt'
#tag_file_name = '/home/shobi/Thesis/' + new_folder_name + '/BH_tag_' + new_file_name + '.txt'
data_file_name = '/home/shobi/Thesis/' + new_folder_name + '/BH_data_' + new_file_name + '.txt'
...
class Datum(object):
    colorin = mcolors.to_rgba("green")
    colorout_pbmc_l = mcolors.to_rgba("black")
    colorout_thp1 = mcolors.to_rgba('red')
    colorout_pbmc_m = mcolors.to_rgba('blue')
    colorout_pbmc_u = mcolors.to_rgba('yellow')
    colourout_unknown = mcolors.to_rgba('purple')

    def __init__(self, x, y, label, tag, include=False):
        self.x = x
        self.y = y
        self.label = label
        self.tag = tag[0]
        ...
        if include:
            ...
            if self.label == 1:
                self.color = self.colorout_thp1
                self.sizes = 8
            elif self.label == 0:
                self.sizes = 3
                if 'Pu' in self.tag:
                    self.color = self.colorout_pbmc_u
                elif 'Plym' in self.tag:
                    self.color = self.colorout_pbmc_l
                else:
                    self.color = self.colorout_pbmc_m
            # elif self.label ==1:
            # self.color = self.colorout_thp1
            else:
                self.color = self.colourout_unknown
...
class LassoManager(object):
    def __init__(self, ax, data):
        ...
        facesizes = [d.sizes for d in data]
        self.xys = [(d.x, d.y) for d in data]
        self.labels = [d.label for d in data]
        self.tags = [d.tag for d in data]
    ...
    def callback(self, verts):
        ...
        for i in range(len(self.xys)):
            if ind[i]:
                ...
                # print(self.xys[i], self.labels[i], self.tags[i])
                xys_list.append(self.xys[i])
                labels_list.append(self.labels[i])
                tags_list.append(self.tags[i])
            else:
                if self.labels[i] == 0:
                    if 'Pu' in self.tags[i]:
                        facecolors[i] = Datum.colorout_pbmc_u
                    elif 'Plym' in self.tags[i]:
                        facecolors[i] = Datum.colorout_pbmc_l
                    else:
                        facecolors[i] = Datum.colorout_pbmc_m
                if self.labels[i] == 1:
                    facecolors[i] = Datum.colorout_thp1
            # ===================================================================
            # else:
            #     if self.labels[i]==0:
            #         facecolors[i] = Datum.colorout_pbmc
            #     if self.labels[i]==1 :
            #         facecolors[i] = Datum.colorout_thp1
            # ===================================================================

        # np.reshape(label, (label.shape[0],1))
        S = np.hstack((np.array(xys_list), np.reshape(np.array(labels_list), (len(labels_list), 1)),
                       np.reshape(np.array(tags_list), (len(tags_list), 1))))
        print(S)
