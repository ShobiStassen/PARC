...
def run_kmeans(X_embedded, n_cancer, true_label,init_peaks, data_version, tsne_version, embedding_filename, tsne_runtime):
    ...
    vals_roc, predict_class_array = accuracy_mst(model_kmeans, true_label[:, 0],
...
def run_dbscan(X_embedded, n_cancer, true_label, data_version, tsne_version, embedding_filename, tsne_runtime, mu_edge,std_edge):
...
def run_mstclustering(X_embedded, n_cancer, true_label, data_version, tsne_version, embedding_filename, tsne_runtime):
    ...
    sigma_list = [3,2.5,2]
    tooclosefactor_list = [30,25,20,15,10,0]
    if n_cancer > 1000:
        cluster_size_list = [20]
        min_cluster_size = 20
    else:
    ...
    for i_sigma in sigma_list:
        for i_tooclosefactor in tooclosefactor_list:
            ...
            iclus = 1
    mu_edge = model.mean_edge_distance
    std_edge = model.std_edge_distance
    df_accuracy = pd.DataFrame(list_roc,
    ...
    return predict_class_aggregate, df_accuracy, sigma_opt, min_cluster_size_opt,mergetooclose_opt,mu_edge, std_edge
...
def accuracy_mst(model, true_labels, embedding_filename, clustering_algo):
    ...
    if clustering_algo=='phenograph': mst_runtime = None
...
def plot_mst_simple(model, true_labels, embedding_filename, embedding_plot_title, sigma, min_cluster,
                    cancer_type, clustering_method,df_all, feature1_name, feature2_name, X_embedded=None):
    ...
    X_dict = {}
    Index_dict = {}
    if clustering_method == 'phenograph': X_plot = X_embedded
...
def run_main(cancer_type, n_cancer, tsne_algo):
    ...
    if method == 'lv':
        ...
        perp_range = [30]  # [10,30,50,100,150]
    if method == 'bh':
        lr = 1000
        perplexity = 30
    ...
    for dataset_version in dataset_version_range:
        ...
        accuracy_mst(communities, true_label, 'embedding_filename', 'phenograph')
        ...
        for perplexity in perp_range:
            ...
            for tsne_version in tsne_version_range:
                ...
                predict_class_aggregate, df_accuracy, sigma_opt, min_cluster_size_opt, tooclosefactor_opt, mu_edge, std_edge = run_mstclustering(
                ...
                model_mst = MSTClustering(cutoff_scale=0.3, approximate=True, min_cluster_size=min_cluster_size_opt,
                ...
                model_kmeans = KMeans(n_clusters=peak_vals.shape[0], init=peak_vals, n_init=1, n_jobs=-3).fit(
                    X_embedded, 9)
                print('end KMEANS')
                plot_mst_simple(communities, true_label[:, 0], embedding_filename, embedding_plot_title, sigma=None,
                                min_cluster=None, cancer_type=cancer_type, clustering_method='phenograph',
                                df_all=df_all,
                                feature1_name=feature1_name, feature2_name=feature2_name, X_embedded=X_embedded)
                list_accuracy_kmeans.append(
                    plot_mst_simple(model_kmeans, true_label[:, 0], embedding_filename, embedding_plot_title,
                                    sigma=None,
                                    min_cluster=None, cancer_type=cancer_type, clustering_method='kmeans',
                                    df_all=df_all,
                                    feature1_name=feature1_name, feature2_name=feature2_name))
                list_accuracy_opt1.append(
                    plot_mst_simple(model_mst, true_label[:, 0], embedding_filename, embedding_plot_title, sigma_opt,
                                    min_cluster_size_opt, cancer_type, clustering_method='mst', df_all=df_all,
                                    feature1_name=feature1_name, feature2_name=feature2_name) + auc_list)
                ...
                if n_total < 500000:
                    ...
                    list_accuracy_opt_dbscan.append(
                        plot_mst_simple(model_dbscan, true_label[:, 0], embedding_filename, embedding_plot_title,
                                        eps_opt, min_cluster_size_opt_dbscan, cancer_type, clustering_method='dbscan',
                                        df_all=df_all, feature1_name=feature1_name,
                                        feature2_name=feature2_name) + auc_list_dbscan)
