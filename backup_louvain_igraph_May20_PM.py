...
def get_data(cancer_type, benign_type, n_cancer, ratio, fluor, dataset_number, new_folder_name, method, randomseedval):
    ...
    new_file_name = new_file_name_title = 'N' + str(n_total) + '_r{:.2f}'.format(
        ratio) + cancer_type + '_pbmc_gated_d' + str(
        dataset_number)
    if method == 'bh':
    ...
    if benign_type == 'pbmc':
        ...
        df_pbmc['cell_tag'] = 'pbmc2017Nov22_' + pbmc_fileidx["filename"].map(int).map(str) + 'midx' + pbmc_fileidx[
    ...
    if cancer_type == 'k562':
        ...
        df_k562['cell_idx_inmatfile'] = k562_fileidx["matlabindex"].map(int).map(
            str)  # should be same number as image number within that folder
...
def run_toobig_sublouvain(X_data, knn_struct, k_nn, self_loop=False, keep_all=False, jac_std=0.3,
                          jac_weighted_edges=False):  # 0.3 was default dist_std
    n_elements = X_data.shape[0]
    time_start_knn = time.time()
    ...
    if self_loop == False:
        csr_array, undirected_graph_array_forLV = make_csrmatrix_noselfloop(neighbor_array, distance_array,
                                                                            keep_all=keep_all,
                                                                            dist_std=0)  # was dist_std = default (which is 0 )until Oct11
    ...
    if jac_std == 'median':
        ...
        threshold = np.mean(sim_list) - jac_std * np.std(sim_list)
    strong_locs = np.where(sim_list_array > threshold)[
        0]  # if it gets smaller than 0.5 then you get too many tiny tiny clusters of size 2
...
def run_jaccard_parallel(graphG, edge_list):
    # returns a list
    return graphG.similarity_jaccard(pairs=edge_list)


def run_sublouvain(X_data, true_label, knn_struct, k_nn, self_loop=False, too_big_factor=0.15, small_pop=5, dist_std=0,
                   keep_all_dist=False, jac_std=0.3, jac_weighted_edges=False, n_iter=10):  # 50
    ...
    if self_loop == False:
        csr_array, undirected_graph_array_forLV = make_csrmatrix_noselfloop(neighbor_array, distance_array, dist_std,
                                                                            keep_all_dist=keep_all_dist)
        # print(undirected_graph_array_forLV)
    ...
    # sim_list = [pool.apply(run_jaccard_parallel, args = (G, sublist)) for sublist in set_edgelists]
    # pool.close()
    # print('simlist', len(sim_list[0:2]))
    # print('simlist', sim_list[350:400])
    ...
    if jac_weighted_edges == True:
        ...
        print('call leiden weighted for ', n_iter, 'iterations')
        ...
        start_leiden = time.time()
        print('call leiden NOT weighted', n_iter, 'iterations')
        partition = leidenalg.find_partition(G_sim, leidenalg.ModularityVertexPartition,
                                             n_iterations=n_iter)  # louvain.find_partition(G_sim, louvain.ModularityVertexPartition, weights='weight') #leidenalg.find_partition(G_sim, leidenalg.ModularityVertexPartition, weights='weight')
        # print('louvain alg')
        # partition = louvain.find_partition(G_sim, louvain.ModularityVertexPartition, n_iterations =5)
        print(time.time() - start_leiden)
        # partition = louvain.find_partition(G_sim, louvain.ModularityVertexPartition)
    ...
    print('finished calling leiden at', time.asctime(), len(partition), 'clusters', 'time elapsed on leiden call ',
          round(time.time() - time_start_louvain))
    # print('members',partition)
    small_pop_list = []
    small_cluster_list = []
    ...
    cluster_i_loc = np.where(louvain_labels == 0)[
    ...
    if too_big == True: print('removing too big, cluster, ', cluster_too_big, 'with population ', list_pop_too_bigs[0],
                              'among list of toobig clusters:', list_pop_too_bigs)
    while too_big == True:
        ...
        louvain_labels_big = run_toobig_sublouvain(X_data_big, knn_struct_big, k_nn=knn_big, self_loop=False,
                                                   jac_std=jac_std)  # knn=200 for 10x
    ...
    return louvain_labels, undirected_graph_array_forLV, time_end_louvain, time_end_prune, time_knn_query_end, len(
        strong_locs)


def run_mainlouvain(X_data, true_label, self_loop=False, LV_graphinput_file_name=None, LV_plot_file_name=None,
                    too_big_factor=0.4, knn_in=30, small_pop=10, dist_std=1, keep_all_dist=True, jac_std=0.3,
    ...
    for k_nn in k_nn_range:
        ...
        time_start_louvain = time.time()
        louvain_labels, undirected_graph_array_forLV, time_end_louvain, time_end_prune, time_end_knn_query, num_edges = run_sublouvain(
            X_data, true_label, knn_struct, k_nn, self_loop=self_loop, too_big_factor=too_big_factor,
            small_pop=small_pop, dist_std=dist_std, keep_all_dist=keep_all_dist, jac_std=jac_std,
        ...
        for onevsall_val in targets:
            ...
            vals_roc, predict_class_array, majority_truth_labels, numclusters_targetval = accuracy_mst(
                list(louvain_labels.flatten()), true_label,
            ...
            list_roc.append(
                [ef, k_nn, jac_std, dist_std, onevsall_val] + vals_roc + [numclusters_targetval] + [run_time])
    ...
    df_accuracy = pd.DataFrame(list_roc,
                               columns=['ef', 'knn', 'jac_std', 'dist_std', 'onevsall-target', 'error rate', 'f1-score',
                                        'tnr', 'fnr',
                                        'tpr', 'fpr', 'precision', 'recall', 'num_groups', 'population of target',
                                        'num clusters', 'clustering runtime'])
...
def run_phenograph(X_data, true_label, knn=30):
    ...
    f1_acc_noweighting = 0
    for onevsall_val in targets:
        vals_roc, predict_class_array, majority_truth_labels, numclusters_targetval = accuracy_mst(list(communities),
                                                                                                   pd.Series(
                                                                                                       true_label),
                                                                                                   None, 'phenograph',
    ...
    df_accuracy = pd.DataFrame(list_roc, columns=['onevsall-target', 'error rate', 'f1-score', 'tnr', 'fnr',
                                                  ...
                                                  'population of class/group', 'num clusters for that class',
                                                  'clustering runtime', 'knn'])
    ...
    return predict_class_aggregate, df_accuracy, list(
        communities), onevsall_opt, majority_truth_labels, pheno_time, f1_acc_noweighting
...
def accuracy_mst(model, true_labels, embedding_filename, clustering_algo, phenograph_time=None, onevsall=1,
    ...
    # else: X = model.X_fit_
    # print(X.shape)
    if clustering_algo == 'phenograph':
        mst_labels = list(model)
    ...
    if clustering_algo == 'phenograph':
        mst_runtime = phenograph_time
    elif clustering_algo == 'multiclass mst' or clustering_algo == 'louvain':
        mst_runtime = None

    else:
        mst_runtime = model.clustering_runtime_
    if clustering_algo == 'louvain' or clustering_algo == 'phenograph' or clustering_algo == 'multiclass mst' or clustering_algo == 'kmeans':
        accuracy_val = [error_rate, f1_score, tnr, fnr, tpr, fpr, precision,
                        recall, num_groups, n_target]
    else:
        accuracy_val = [embedding_filename, sigma, min_cluster_size, mergetooclosefactor, error_rate, f1_score, tnr,
                        fnr, tpr, fpr, precision,
...
def run_main(cancer_type, n_cancer, n_benign, randomseedval=[1, 2, 3]):
    ...
    for dataset_version in dataset_version_range:
        ...
        true_label, tag, X_data, new_file_name, df_all, index_list, flist = get_data(cancer_type, benign_type, n_cancer,
                                                                                     ...
                                                                                     randomseedval=randomseed_singleval)
        ...
        predict_class_aggregate_louvain, df_accuracy_louvain, best_louvain_labels, knn_opt, onevsall_opt, maj_vals = run_mainlouvain(
            X_data,
