...
def run_dbscan(X_embedded, n_cancer, true_label, data_version, tsne_version, embedding_filename, tsne_runtime, mu_edge,std_edge):
    ...
    list_roc = []
    sigma_list = [0.4, 0.5] #eps=0.5 is the default, we are setting eps as mu+2.5*std of edge distance in mst_graph of embedding
    ...
    for dataset_version in dataset_version_range:
        ...
        for perplexity in perp_range:
            ...
            for tsne_version in tsne_version_range:
                ...
                if n_total < 500000:
                    predict_class_aggregate_dbscan, df_accuracy_dbscan, eps_opt, min_cluster_size_opt_dbscan, tooclosefactor_opt = run_dbscan(X_embedded, n_cancer, true_label, dataset_version, tsne_version, embedding_filename, tsne_runtime, mu_edge,std_edge)
                ...
                print('start KMEANS')
                model_kmeans = KMeans(n_clusters=peak_vals.shape[0], init=peak_vals, n_init=1, n_jobs=-3).fit(X_embedded,9)
                print('end KMEANS')
                list_accuracy_kmeans.append(
                    plot_mst_simple(model_kmeans, true_label[:, 0], embedding_filename, embedding_plot_title, sigma=None,
                                    min_cluster=None, cancer_type=cancer_type, clustering_method='kmeans', df_all=df_all,
                                    feature1_name=feature1_name, feature2_name=feature2_name))
                list_accuracy_opt1.append(
                    plot_mst_simple(model_mst, true_label[:, 0], embedding_filename, embedding_plot_title,sigma_opt,min_cluster_size_opt, cancer_type,clustering_method='mst',df_all = df_all,feature1_name = feature1_name,feature2_name=feature2_name) + auc_list)
                print('auc list', auc_list)
                eps_opt = model_mst.mean_edge_distance + sigma_opt*model_mst.std_edge_distance
                eps_opt = max(eps_opt, 0.5)
                if n_total < 500000:
                    model_dbscan = DBSCAN(eps_opt, min_cluster_size_opt_dbscan, tooclose_factor = tooclosefactor_opt).fit(X_embedded)
        ...
        df_accuracy_opt1 = pd.DataFrame(list_accuracy_opt1,
                                        columns=['embedding filename', 'sigma', 'min cluster size', 'merge too close factor','f1-score', 'tnr',
