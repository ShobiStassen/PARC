...
def run_toobig_sublouvain(X_data, knn_struct,k_nn,self_loop = False, keep_all=False,jac_std= 0.3): #0.3 was default dist_std
    ...
    if self_loop == False:
        if keep_all_dist== False:
            csr_array, undirected_graph_array_forLV = make_csrmatrix_noselfloop(neighbor_array, distance_array,keep_all_dist= keep_all)
            sources, targets = csr_array.nonzero()
            edgelist = list(zip(sources.tolist(), targets.tolist()))
            G = ig.Graph(edgelist)#, edge_attrs={'weight': csr_array.data.tolist()})
        else:
            from itertools import chain
            targets = neighbor_array.flatten()
            sources = list(chain.from_iterable([[i]*k_nn for i in range(n_elements)]))
    ...
    #if keep_all_dist== True:
    #    new_edgelist = edgelist_copy
    #    sim_list_new = sim_list
    ...
    new_edgelist = [edgelist_copy[ii] for ii in strong_locs]
...
def run_sublouvain(X_data, knn_struct,k_nn,self_loop = False, too_big_factor = 0.15, small_pop = 5, dist_std = 0,keep_all= False,jac_std= 0.3):#50
    ...
    if self_loop == False:
        if keep_all_dist== False:
            csr_array, undirected_graph_array_forLV = make_csrmatrix_noselfloop(neighbor_array, distance_array, dist_std,keep_all_dist= keep_all)
            sources, targets = csr_array.nonzero()
            edgelist = list(zip(sources.tolist(), targets.tolist()))
            G = ig.Graph(edgelist)#, edge_attrs={'weight': csr_array.data.tolist()})
        else:
            from itertools import chain
            targets = neighbor_array.flatten()
            #print(len(targets),'len targets', targets[20:40])
            sources = list(chain.from_iterable([[i]*k_nn for i in range(n_elements)]))
            #print(len(sources), 'len sources', sources[20:40])
            edgelist = list(zip(sources,targets))
            edgelist = [(s,t) for (s,t) in edgelist if s!=t]
            #print(edgelist[20:40], 'length of edgelist is', len(edgelist))
            G = ig.Graph(edgelist)#, edge_attrs={'weight': csr_array.data.tolist()})
    ...
    print('len of sim-list before pruning', len(sim_list))
    #print('pre-lognormed mean and std', np.mean(sim_list), np.std(sim_list))
    #plt.hist(sim_list)
    #plt.show()
    #sim_list = stats.zscore(boxcox(sim_list,0))#log to get normal distribution
    ...
    #if keep_all_dist== True:
     #   new_edgelist = edgelist_copy
        #sim_list_new = sim_list.copy()
    #else:
    strong_locs = np.where(sim_list_array>np.mean(sim_list) - Jac_std*np.std(sim_list))[0] #if it gets smaller than 0.5 then you get too many tiny tiny clusters of size 2
    for ii in strong_locs: new_edgelist.append(edgelist_copy[ii])
    sim_list_new = list(sim_list_array[strong_locs])
    print('percentage of edges KEPT', len(strong_locs) / len(sim_list), len(sim_list_new)/len(edgelist))
    ...
    #optimiser.consider_comms = louvain.RAND_NEIGH_COMM

    print('complete louvain partition')
    partition = louvain.find_partition(G_sim, louvain.ModularityVertexPartition)#, weights='weight') #if you want to partition by incroporating weights, you need to explicity set weights = 'weight'.
    louvain_labels = np.empty((n_elements, 1))
    ...
    while too_big == True:
        ...
        print('calling run_toobig()')
        louvain_labels_big = run_toobig_sublouvain(X_data_big, knn_struct_big, k_nn=knn_big, self_loop=False,Jac_std=Jac_std,keep_all=keep_all) #knn=200 for 10x
...
def run_mainlouvain(X_data,true_label, self_loop = False,LV_graphinput_file_name =None, LV_plot_file_name =None, too_big_factor = 0.4, knn_in = 30,small_pop = 50,dist_std = 0, keep_all_dist= False,Jac_std=0.3):
    list_roc = []
    ...
    for k_nn in k_nn_range:
        ...
        louvain_labels, undirected_graph_array_forLV,time_end_louvain, time_end_prune = run_sublouvain(X_data, knn_struct, k_nn, self_loop=self_loop, too_big_factor=too_big_factor, small_pop = small_pop, dist_std = dist_std, keep_all_dist= keep_all,Jac_std=Jac_std)
        ...
        for onevsall_val in targets:
            ...
            vals_roc, predict_class_array, majority_truth_labels = accuracy_mst(list(louvain_labels.flatten()), true_label,
        ...
        print("f1-score mean ", f1_acc_noweighting, f1_acc_noweighting / len(targets))
    ...
    return predict_class_aggregate, df_accuracy, temp_best_labels,knn_opt, onevsall_opt, majority_truth_labels, time_end_knn, time_end_prune, time_end_louvain, time_end_total
...
def run_phenograph(X_data, true_label):
    ...
    f1_acc_noweighting = 0
    for onevsall_val in targets:
        vals_roc, predict_class_array, majority_truth_labels = accuracy_mst(list(communities), true_label,
                                                        None, 'phenograph',
    ...
    print('Accumulated F1-Score', f1_accumulated)
    print("f1-score mean ", f1_acc_noweighting,f1_acc_noweighting / len(targets))
    return predict_class_aggregate, df_accuracy, list(communities), onevsall_opt, majority_truth_labels
...
def accuracy_mst(model, true_labels, embedding_filename, clustering_algo,phenograph_time = None, onevsall=1,verbose_print = False):
    ...
    for kk in sorted_keys:
        ...
        if verbose_print== True: print(kk, 'is majority ', majority_val, 'with population ', len(vals))
        if kk==-1:
    ...
    for cluster_i in set(mst_labels):
        ...
        #print('majority truth within loop', majority_truth)
    #print(majority_truth_labels)
    majority_truth_labels = list(majority_truth_labels.flatten())
    print(majority_truth_labels[:10])
...
def main():
    ...
    run_main('k562', n_cancer=200, n_benign=300, randomseedval=[10])#[10,11,12])
