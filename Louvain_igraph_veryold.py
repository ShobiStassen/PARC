...
def make_csrmatrix_noselfloop(neighbor_array, distance_array):
    ...
    if n_neighbors >= 10:
        ...
        print('weigth array shape', weight_array.shape)
...
def run_mainlouvain(X_data,true_label, self_loop = False,LV_graphinput_file_name =None, LV_plot_file_name =None):
    list_roc = []
    k_nn_range = [20]#[30, 20, 10,5,3]#,30]#,15,20,30]
    ef = 50
    f1_temp = -1
    temp_best_labels = []
    iclus = 0
    for k_nn in k_nn_range:
        ...
        for onevsall_val in target_range:
            ...
            if vals_roc[0] > f1_temp:
                f1_temp = vals_roc[0]
                temp_best_labels = list(louvain_labels.flatten())#[0]
                print(temp_best_labels)
...
def run_main(cancer_type, n_cancer,n_benign):
    ...
    for dataset_version in dataset_version_range:
        ...
        predict_class_aggregate_louvain, df_accuracy_louvain, best_louvain_labels, knn_opt, onevsall_opt = run_mainlouvain(X_data,
                                                                                                             true_label, self_loop=False,LV_graphinput_file_name = LV_graphinput_file_name, LV_plot_file_name = LV_plot_file_name)
...
def main():
    ...
    run_main('k562', n_cancer=101, n_benign=200)
