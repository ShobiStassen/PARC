...
#THIS IS THE VERSION USED FOR ALL THE LUNG CANCER CELL LINES THAT MADE THE MIDTERM REVIEW RESULTS
"""
...
class MSTClustering3D(BaseEstimator, ClusterMixin):
    ...
    def __init__(self, min_cluster_size=1,
                 ...
                 max_labels=None, true_label = None):
    ...
    def too_close(self, X_dict_mean, X_dict_label, labels, mean, cur_tooclosefactor):
        ...
        for key1 in X_dict_mean.keys():
            # print('key1 is ', key1)
            mindist = 999 #arbitrary large value
            ...
            if (mindist > 0) and (mindist < cur_tooclosefactor * mean):  # 10
                #print('inside too close function, cur_tooclose multiple is', cur_tooclosefactor)
    ...
    def fit(self, X, y=None):
        ...
        if self.metric == 'precomputed':
            ...
            if X.shape[0]< 2000:
                ...
                #Plotting_3D.save_anim_graph(edgelist, X, self.true_label, graph_title='5-NN Graph_'+str(len(self.true_label)))
        ...
        self.full_tree_ = minimum_spanning_tree(G, overwrite=True)
            ...
            #edgelist = zip(sources.tolist(), targets.tolist())
            #Plotting_3D.save_anim_graph(edgelist, X, self.true_label, graph_title='MST Graph_' + str(len(self.true_label)))
        #fig_graph = plt.figure(figsize=(24, 24))
        #ax = fig_graph.add_subplot(1, 1, 1, projection='3d')
        #Plotting_3D.plot_graph_3D(fig_graph=fig_graph,ax=ax,edgelist=edgelist,X=X, true_label=self.true_label)
        '''
        ...
        if max_labels < 15: #until sep12 was 10
