...
def run_mctsne(version, input_data,perplexity, lr, new_file_name, new_folder_name):
    ...
    embedding_filename = '/home/shobi/Thesis/BarnesHutMC_data_MinCluster/'+new_folder_name+'/BH_embedding' +  new_file_name +'_lr'+str(lr)+'_perplexity'+str(perplexity)+'_bh'+str(version)+ '.txt'
...
def run_mstclustering(X_embedded, n_cancer, true_label, data_version, tsne_version, embedding_filename, tsne_runtime,df_all):
    list_roc = []
    ...
    model = MSTClustering3D(approximate=True, min_cluster_size=min_cluster_size, max_labels=av_peaks)
    time_start = time.time()
    ...
    print('Clustering Done! Time elapsed: {:.2f} seconds'.format(mst_runtime),
          'tsne version {}'.format(tsne_version), 'data version {}'.format(data_version),
          'sigma {}'.format(model.sigma_factor),'tooclosefactor {}'.format(model.tooclosefactor), 'and min cluster {}'.format(min_cluster_size))
    ...
    return predict_class_aggregate, df_accuracy, sigma_opt, min_cluster_size_opt,mergetooclose_opt, av_peaks
...
def plot_mst_simple(ax0, ax1,model, true_labels, embedding_filename, embedding_plot_title, sigma, min_cluster,
    ...
    ax0.scatter(X_plot[idx_cancer, 0], X_plot[idx_cancer, 1], color='red', zorder=2,alpha=0.5, s=4)
    ...
    for color_t, ll_t in zip(colors_thp1, thp1_labels):
        ...
        ax1.scatter(x, y, color=color_t, s=4, alpha=0.5,
...
def tsne_densityplot(X_embedded,ax1,ax2,df_all, mode = 'plot'):
    ...
    peak_coord = plot_peaks(X, Y,positions, Z_tsne, ax1, ax2, density_type='tsne', mode = mode) #returns n_peak x2 array
...
def run_main(cancer_type, n_cancer, n_benign,tsne_algo, randomseed=None):
    ...
    benign_type = 'pbmc'
    fluor = 0
    num_dataset_versions = len(randomseed)
    num_tsne_versions = 1
    ...
    for dataset_version in dataset_version_range:
        if method == 'lv':
            excel_file_name = '/home/shobi/Thesis/LV_data_MinCluster/' + new_folder_name + '/LV_excel_' + cancer_type + '_data' + str(
                dataset_version) + '_r{:.2f}'.format(ratio) + '_ncancer' + str(n_cancer) +'_rs'+ str(randomseed[dataset_version])+ '.xlsx'
        else:
        ...
        for perplexity in perp_range:
            ...
            for tsne_version in tsne_version_range:
                ...
                predict_class_aggregate, df_accuracy, sigma_opt, min_cluster_size_opt, tooclosefactor_opt, av_peaks = run_mstclustering(
                    X_embedded, n_cancer, true_label, dataset_version, tsne_version, embedding_filename, tsne_runtime, df_all)
                if n_total < 500000:
                ...
                model_mst = MSTClustering3D(approximate=True, min_cluster_size=min_cluster_size_opt, max_labels=av_peaks)
...
def main():
    ...
    pb = 46*1001
    rs = [10]
    print(rs,'is the randomseed list')
    #run_main('k562', 46, pb, 'bh', randomseed=[23, 21, 22])
    run_main('k562', 46, pb, 'lv', randomseed=rs)
