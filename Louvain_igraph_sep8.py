...
def make_csrmatrix_noselfloop(neighbor_array, distance_array):
    ...
    for row in neighbor_array:
        ...
        #print(distlist)
        to_keep = np.where(distlist < np.mean(distlist)+0*np.std(distlist))[0] #0*std
        ...
        for ik in range(len(updated_nn_ind)):
            if rowi != row[ik]:  # remove self-loops
                ...
                if dist ==0:
                    #print(distlist)
                    dist = (np.mean(distlist) - 3*np.std(distlist))*0.01#0.000001
...
def run_sublouvain(X_data, knn_struct,k_nn,self_loop = False, too_big_factor = 0.4, small_pop = 50):
    n_elements = X_data.shape[0]
    time_start_knn = time.time()
    X_data_copy = copy.deepcopy(X_data)
...
def run_mainlouvain(X_data,true_label, self_loop = False,LV_graphinput_file_name =None, LV_plot_file_name =None, too_big_factor = 0.4, knn_in = 30,small_pop = 50):
    list_roc = []
    ...
    for k_nn in k_nn_range:
        ...
        louvain_labels, undirected_graph_array_forLV = run_sublouvain(X_data, knn_struct, k_nn, self_loop=self_loop, too_big_factor=too_big_factor, small_pop = small_pop)
