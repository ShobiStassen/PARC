...
#This is the vereion you can use to re-create the results from the 3-professor PPT in Oct1
import numpy as np
import sys
...
class MSTClustering3D(BaseEstimator, ClusterMixin):
    ...
    def __init__(self, min_cluster_size=1,
                 ...
                 max_labels=None, true_label=None,precomputed = False):
    ...
    def too_close(self, X_dict_mean, X_dict_label, labels, mean, cur_tooclosefactor):
        ...
        for key1 in X_dict_mean.keys():
            ...
            for key2 in X_dict_mean.keys():
                (x1, y1, z1) = X_dict_mean[key1][0]
                (x2, y2, z2) = X_dict_mean[key2][0]
    ...
    def fit(self, X, y=None):
        ...
        if self.metric == 'precomputed':
            ...
            for k in range(X.shape[0]):
                ...
                y = X[k, 1]
                z = X[k, 2]
                X_dict.setdefault(k, np.array([x, y, z]))
            '''    
            if X.shape[0] < 2000:
        ...
        if max_labels < 10:
            ...
            cur_tooclosefactor = 0
            updated = True
            while len(set(labels)) < 750 and cur_tooclosefactor <= 50:# was 30 until sep 13 ,was 500 until 21sept
                updated = True
                while updated == True:
                    X_dict_mean, X_dict, labels, updated = self.too_close(X_dict_mean, X_dict, labels, mu,
                                                                          cur_tooclosefactor=cur_tooclosefactor)
        ...
        cur_tooclosefactor = 0
        updated = True
        while len(set(labels)) < 750 and cur_tooclosefactor <= 50:  # was 500 until 21sept
            updated = True
            while updated == True:
                X_dict_mean, X_dict, labels, updated = self.too_close(X_dict_mean, X_dict, labels, mu,
                                                                      cur_tooclosefactor=cur_tooclosefactor)
