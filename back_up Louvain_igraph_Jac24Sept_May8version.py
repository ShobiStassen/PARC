...
def run_sublouvain(X_data, true_label, knn_struct,k_nn,self_loop = False, too_big_factor = 0.15, small_pop = 5, dist_std = 0,keep_all_dist= False,jac_std= 0.3, jac_weighted_edges = False):#50
    ...
    if jac_weighted_edges == True:
        ...
        print('call leiden')
        partition = leidenalg.find_partition(G_sim, leidenalg.ModularityVertexPartition, weights='weight', n_iterations = 5)
        #print('louvain alg')
        ...
        start_leiden = time.time()
        print('call leiden')
        partition = leidenalg.find_partition(G_sim, leidenalg.ModularityVertexPartition,n_iterations =5)  # louvain.find_partition(G_sim, louvain.ModularityVertexPartition, weights='weight') #leidenalg.find_partition(G_sim, leidenalg.ModularityVertexPartition, weights='weight')
        #print('louvain alg')
    ...
    print('finished calling louvain at',time.asctime(),len(partition),'clusters','time elapsed on louvain call ', round(time.time()-time_start_louvain))
    ...
    print('small and big round 2', round(time.time()-time_end_louvain), time.ctime())
    dummy, louvain_labels = np.unique(list(louvain_labels.flatten()), return_inverse=True)
    louvain_labels=np.asarray(louvain_labels)
