...
def make_knn_struct(X_data, ef=100):
...
def make_csrmatrix_noselfloop(neighbor_array, distance_array):
    ...
    for row in neighbor_array:
        ...
        #print(distlist)
        to_keep = np.where(distlist < np.mean(distlist)+1*np.std(distlist))[0] #0*std
        ...
        for ik in range(len(updated_nn_ind)):
            if rowi != row[ik]:  # remove self-loops
                ...
                if dist ==0:
                    ...
                    print('rowi and row[ik] are modified to', rowi, row[ik], 'and are',dist, 'dist apart')
...
def run_sublouvain(X_data, knn_struct,k_nn,self_loop = False, too_big_factor = 0.4):
    n_elements = X_data.shape[0]
    time_start_knn = time.time()
    X_data_copy = copy.deepcopy(X_data)
    ...
    for key in range(len(partition)):
        ...
        for cell in partition[key]:
            louvain_labels[cell] = key
    print('list of clusters that have low populiation:', small_pop_list)
...
def run_mainlouvain(X_data,true_label, self_loop = False,LV_graphinput_file_name =None, LV_plot_file_name =None, too_big_factor = 0.4, knn_in = 30):
    list_roc = []
    ...
    for k_nn in k_nn_range:
        ...
        time_start_louvain = time.time()
        print(time_start_louvain, 'knn and toobig factor are', k_nn, too_big_factor)
        louvain_labels, undirected_graph_array_forLV = run_sublouvain(X_data, knn_struct, k_nn, self_loop=self_loop, too_big_factor=too_big_factor)
