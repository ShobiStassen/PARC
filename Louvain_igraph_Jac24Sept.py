...
def make_knn_struct(X_data, ef=100, knn = 30, too_big=False, big_cluster=False):#100
    if knn> 190: print('please provide a lower K_in for KNN graph construction')
    ef_query = max(100, knn + 1)  # ef always should be >K. higher ef, more accuate query
    if too_big == False:
        num_dims = X_data.shape[1]
        n_elements = X_data.shape[0]
        p = hnswlib.Index(space='l2', dim=num_dims)
        if n_elements < 1000:
            ef_param_const = n_elements
            ef_query = n_elements
            print('setting ef_construction to', ef_param_const)
        else:
            ef_param_const = 200
        if num_dims > 30:
    ...
    if too_big == True:
        ...
        p = hnswlib.Index(space='l2', dim=num_dims)
        p.init_index(max_elements=n_elements, ef_construction=200, M=200)
        p.init_index(max_elements=n_elements, ef_construction=200, M=30)
        p.add_items(big_cluster)
    print('ef_query is', ef_query)
...
def make_csrmatrix_noselfloop(neighbor_array, distance_array,dist_std =0, keep_all_dist= False):
    ...
    if keep_all_dist== True: #dont prune based on distance
        ...
        #distance_array[distance_array==0]= 0.0000001 #making the zero dist (i.e. self loops) non-zero
        #weight_list = distance_array.flatten().tolist()
    print('lengths:',len(weight_list), len(row_list),len(col_list))
    ...
    '''
    #print('time to make csr matrix no selfloop', round(time.time() - time_temp),'secs')
...
def run_toobig_sublouvain(X_data, knn_struct,k_nn,self_loop = False, keep_all=False,jac_std= 0.3,jac_weighted_edges = False): #0.3 was default dist_std
    ...
    if self_loop == False:
        csr_array, undirected_graph_array_forLV = make_csrmatrix_noselfloop(neighbor_array, distance_array, keep_all_dist=keep_all, dist_std=0)#was dist_std = default (which is 0 )until Oct11
    ...
    #print('time to make igraph:', round(time.time()-time_temp),'secs')
    time_temp = time.time()
    #print('computing Jaccard metric', time.ctime())
    sim_list = G.similarity_jaccard(pairs=edgelist_copy) #list of jaccard weights
    #print('time to compute jaccards:', round(time.time() - time_temp), 'secs')
    ...
    print('jac threshold', threshold)
    print('jac std', np.std(sim_list))
    print('jac mean', np.mean(sim_list))
    ...
    print('time take to make parition into labels', round(time.time() - time_labels))
    print('list of clusters that have low populiation:', small_cluster_list)
    time_small_pop = time.time()
    for key in range(len(partition)):
        population = len(partition[key])
        if population < 10:  # 10 for cytof
            #print(key, ' has small population of ', population, )
            small_pop_list.append([t for t in partition[key]])
            small_cluster_list.append(key)
    print('time taken for small_cluster detection', round(time.time() - time_small_pop))
...
def run_sublouvain(X_data, true_label, knn_struct,k_nn,self_loop = False, too_big_factor = 0.15, small_pop = 5, dist_std = 0,keep_all_dist= False,jac_std= 0.3, jac_weighted_edges = False, n_iter_leiden=5, partition_seed=0,parc_method ='leiden'):#50
    ...
    # edgelist1 = list(zip(sources[0:40000],targets[0:40000]))
    # edgelist2 = list(zip(sources[40000:80000],targets[40000:80000]))
    # edgelist3 = list(zip(sources[80000:], targets[80000:]))
    # set_edgelists = []
    # set_edgelists.append(edgelist1)
    # set_edgelists.append(edgelist2)
    # set_edgelists.append(edgelist3)
    # print(len(set_edgelists),len(set_edgelists[1]))

    # #print(len(sources),len(targets))
    # mask = np.zeros(len(sources), dtype=bool)
    # mask |= (csr_array.data > (np.mean(csr_array.data) )+ np.std(csr_array.data)*1)
    # print('sum of mask', sum(mask))
    # csr_array.data[mask] = 0
    # csr_array.eliminate_zeros()
    # sources, targets = csr_array.nonzero()
    ...
    G = ig.Graph(edgelist)#, edge_attrs={'weight': csr_array.data.tolist()})
    print('average degree of initial graph is ', np.mean(G.degree()))
    #retrieve = G.get_edgelist()
    #print(len(retrieve),retrieve[0:5])
    #G.simplify(combine_edges='sum', loops = True)  # "first"


    #cut the jaccard step start
    ...
    # f = Fitter(sim_list, distributions=['gamma'])
    # f.fit()
    # f.summary()
    # print('time to compute jaccards:', round(time.time() - time_temp), 'secs')
    #https://igraph.org/python/doc/igraph.GraphBase-class.html#similarity_jaccard


    #print('time to compute jaccards MP:', round(time.time() - time_temp), 'secs')
    #print('pre-lognormed mean and std', np.mean(sim_list), np.std(sim_list))
    # plt.hist(sim_list, bins=160, range=[0,0.8], color='#6dd18e')
    # plt.xlabel("Jaccard co-efficient")
    # plt.ylabel("No. edges")
    # #plt.title("sc-RNA: PBMC_10X")
    # #plt.title("Mass Cytometry: Samusik_all")
    # plt.title("Flow Cytometry: Mossman_rare")
    # plt.grid(False)
    # #plt.show()

    # plt.hist(sim_list, bins=160, range=[0,0.8], color='#6dd18e', log=True)
    # plt.xlabel("Jaccard co-efficient")
    # plt.ylabel("No. edges")
    # #plt.title("Mass Cytometry: Samusik_all")
    # #plt.title("sc-RNA: PBMC_10X")
    ...
    print(len(sim_list), np.mean(sim_list))
    time_temp = time.time()
    if jac_std=='median': threshold = np.median(sim_list)
    else: threshold = np.mean(sim_list) - jac_std*np.std(sim_list)
    print('jac threshold', threshold)
    ...
    sim_list_new = list(sim_list_array[strong_locs])
    print('time to prune jaccard:', round(time.time()-time_temp),'secs')
    time_temp = time.time()
    ...
    print('time to remake and simplify igraph:', round(time.time() - time_temp), 'secs')
    print('average degree of SIMPLE graph is ', np.mean(G_sim.degree()))
    #end cut jaccard step


    #G_sim = ig.Graph(list(edgelist), edge_attrs={'weight': sim_list})
    #layout = G.layout("rt",2)sm
    #ig.plot(
    #print('average degree of graph is ', np.mean(G_sim.degree()))
    ...
    if jac_weighted_edges == True:
        if parc_method == 'leiden':
            print('call leiden weighted for ',n_iter_leiden, 'iterations')
            partition = leidenalg.find_partition(G_sim, leidenalg.ModularityVertexPartition, weights='weight', n_iterations = n_iter_leiden, seed=partition_seed)
            print(time.time() - start_leiden)
        else:
            print('louvain alg')
            partition = louvain.find_partition(G_sim, louvain.ModularityVertexPartition, weights='weight')
            #louvain.find_partition(G_sim, louvain.ModularityVertexPartition, weights='weight') #leidenalg.find_partition(G_sim, leidenalg.ModularityVertexPartition, weights='weight')
            print(time.time()-start_leiden)
    else:
        if parc_method == 'leiden':
            start_leiden = time.time()
            print('call leiden NOT weighted',n_iter_leiden, 'iterations')
            partition = leidenalg.find_partition(G_sim, leidenalg.ModularityVertexPartition,n_iterations =n_iter_leiden,seed=partition_seed)  # louvain.find_partition(G_sim, louvain.ModularityVertexPartition, weights='weight') #leidenalg.find_partition(G_sim, leidenalg.ModularityVertexPartition, weights='weight')
        else:
            print('louvain alg')
            partition = louvain.find_partition(G_sim, louvain.ModularityVertexPartition) #n_iterations??
            print(time.time() - start_leiden)
    ...
    while li<2:
        time0 = time.time()
        if jac_weighted_edges == True:
            newpart = louvain.find_partition(G, louvain.ModularityVertexPartition, weights='weight')
        else:
            newpart = louvain.find_partition(G, louvain.ModularityVertexPartition)
    ...
    print('finished calling leiden at',time.asctime(),len(partition),'clusters','time elapsed on leiden call ', round(time.time()-time_start_louvain))


    time_labels = time.time()
    louvain_labels = np.asarray(partition.membership)
    louvain_labels = np.reshape(louvain_labels,(n_elements,1))
    print(louvain_labels.shape)
    time_end_louvain = time.time()

    #print('partition.membership', partition.membership)
    # for key in range(len(partition)):
    #     print('key',key)
    #     louvain_labels[partition[key]] = key
        # population = len(partition[key])
        # if population < 10:  # 10 for cytof
        # print(key, ' has small population of ', population, )
        # small_pop_list.append([t for t in partition[key]])
    ...
    for cluster in set(list(louvain_labels.T)[0]):
    ...
    # for key in range(len(partition)):
    #     population = len(partition[key])
    #     if population < small_pop:  # 10 for cytof and other things until April 26
    #         # print(key, ' has small population of ', population, )
    #         small_pop_list.append([t for t in partition[key]])
    #         small_cluster_list.append(key)
    ...
    #         louvain_labels[cell] = key
    # print('list of clusters that have low population:', small_cluster_list)
    #initial pass for small clusters might be bottle neck
    # for small_cluster in small_pop_list:
    #     for single_cell in small_cluster:
    #         #print('single cell')
    #         #print('dim neigh array', neighbor_array.shape)
    ...
    #print('set of louvain labels', set_louvain_labels)
    print('time to start searching for big')
    ...
    if pop_i > too_big_factor * n_elements:  # 0.4
        ...
        cluster_too_big = 0
        print('cluster', 0, 'of population', pop_i)


    found_first_small = False
    cluster_i = 0
    '''
    while found_first_small == False:
    #for cluster_i in range(len(partition)): #partition is listed in order of most populous to least populous. i.e. #1 is most populous...
        #cluster_i_loc =np.where(louvain_labels == cluster_i)[0]
        pop_i = len(partition[cluster_i])#len(cluster_i_loc)
        if pop_i < small_pop:  # 10 for cytof
            first_small_pop = cluster_i
            found_first_small = True
        cluster_i = cluster_i+1
    for cluster_i in list(range(first_small_pop,len(partition))):
        for single_cell in partition[cluster_i]:
        #small_pop_list = small_pop_list + partition[cluster_i]
        #small_cluster_list.append(cluster_i)
    ...
    '''
    #print('small pop list', small_pop_list)
    ...
    for cluster in set(louvain_labels):
        ...
        if population < small_pop:#10
            ...
            small_pop_list.append(list(np.where(louvain_labels==cluster)[0]))
            small_cluster_list.append(cluster)
    print('number of small clusters before initial small_pop handling', len(small_cluster_list))
    #print('list of small clusters Round 2:',small_cluster_list)
    for small_cluster in small_pop_list:
        #print('we will now look at elements of the small cluster', small_cluster)
        for single_cell in small_cluster:
            ...
            #print('best group is', best_group)
    small_loop_check = 0
    len_small_pop_list_prev = 0
    print('small pop exist', small_pop_exist)
    while ((small_pop_exist ==True) & (small_loop_check<100)):
        ...
        for cluster in set(list(louvain_labels.flatten())):
            ...
            if population < small_pop:
                ...
                #print(cluster, ' has small population of', population, )
                small_pop_list.append(np.where(louvain_labels == cluster)[0])
        if len(small_pop_list)== len_small_pop_list_prev: small_loop_check = small_loop_check + 1
        #print('number of small clusters after initial small_pop handling', len(small_pop_list))
        for small_cluster in small_pop_list:
             for single_cell in small_cluster:
                 #print('single cell')
                 #print('dim neigh array', neighbor_array.shape)
                 ...
                 #print(best_group)
                 louvain_labels[single_cell] = best_group
        len_small_pop_list_prev = len(small_pop_list)
        #if small_loop_check>(100-1):print('small loop check',small_loop_check)
...
def run_mainlouvain(X_data,true_label, self_loop = False,LV_graphinput_file_name =None, LV_plot_file_name =None, too_big_factor = 0.4, knn_in = 30,small_pop = 10,dist_std = 1, keep_all_dist= True,jac_std=0.3,jac_weighted_edges = True, n_iter_leiden = 5,partition_seed=0, parc_method='leiden',compute_accuracy=True):
    print('input to parc is', X_data.shape)
    knn_opt =knn_in
    list_roc = []
    ...
    temp_best_labels = []
    iclus = 0
    predict_class_aggregate=[]
    time_start_total = time.time()
    for k_nn in k_nn_range:
        time_start_knn = time.time()
        knn_struct = make_knn_struct(X_data)
        time_end_knn = time.time() - time_start_knn
        print('time elapsed knn', time_end_knn)
        # Query dataset, k - number of closest elements (returns 2 numpy arrays)
        time_start_louvain = time.time()
        parc_labels, undirected_graph_array_forLV,time_end_louvain, time_end_prune, time_end_knn_query, num_edges = run_sublouvain(X_data, true_label, knn_struct, k_nn, self_loop=self_loop, too_big_factor=too_big_factor, small_pop = small_pop, dist_std = dist_std, keep_all_dist= keep_all_dist,jac_std=jac_std, jac_weighted_edges=jac_weighted_edges, n_iter_leiden=n_iter_leiden, partition_seed=partition_seed, parc_method=parc_method)
        ...
        print('PARC total time elapsed {:.2f} seconds'.format(time.time() - time_start_louvain))
        ...
        parc_labels = list(parc_labels.flatten())
        if compute_accuracy ==True:
            for onevsall_val in targets:
                print('target is', onevsall_val)
                vals_roc, predict_class_array, majority_truth_labels, numclusters_targetval = accuracy_mst(parc_labels, true_label,
                                                                 embedding_filename=None, clustering_algo='louvain', onevsall=onevsall_val)
                #list(louvain_labels.T)[0]
                f1_current = vals_roc[1]
                print('target', onevsall_val,'had f1-score of %.3f' % (f1_current*100), "%")
            ...
            df_accuracy = pd.DataFrame(list_roc,
                                       columns=['ef', 'knn', 'jac_std', 'dist_std', 'onevsall-target', 'error rate',
                                                'f1-score', 'tnr', 'fnr',
    ...
    if LV_graphinput_file_name != None:
        ...
        pp.plot_mst_simple(ax[0], ax[1], parc_labels, true_label, None, None, None, None,
    ...
    return predict_class_aggregate, df_accuracy, parc_labels,knn_opt, onevsall_opt, majority_truth_labels, time_end_knn, time_end_prune, time_end_louvain, time_end_total,f1_accumulated,f1_mean, time_end_knn_query, num_edges
...
def run_phenograph(X_data, true_label, knn =30):
    ...
    for onevsall_val in targets:
        ...
        f1_current = vals_roc[1]
        print('f1-score of target', onevsall_val, 'is', f1_current)
...
def accuracy_mst(model, true_labels, embedding_filename, clustering_algo,phenograph_time = None, onevsall=1,verbose_print = False):
    ...
    for kk in sorted_keys:
        ...
        if (majority_val == onevsall) and (kk != -1):
            ...
            print('cluster',kk, 'has error rate of %.2f' % (e_count*100/len(vals)), "%", 'number of non-target cells is', e_count, 'out of a cluster population of', len(vals))
...
def run_main(cancer_type, n_cancer,n_benign, randomseedval=[1,2,3]):
    ...
    for dataset_version in dataset_version_range:
        ...
        predict_class_aggregate_louvain, df_accuracy_louvain, best_louvain_labels, knn_opt, onevsall_opt, maj_vals = run_mainlouvain(X_data,
                                                                                                             true_label, self_loop=False,LV_graphinput_file_name = None, LV_plot_file_name = LV_plot_file_name, parc_method = 'leiden')
